...
$imgnewfile=md5($image1.time()).$extension1;
move_uploaded_file($_FILES["image1"]["tmp_name"],"productimages/".$imgnewfile);
$updatedby=$_SESSION['aid'];
$pid=intval($_GET['id']);
$sql=mysqli_query($con,"update tblrentalcloth set Image1='$imgnewfile', lastUpdatedBy='$updatedby' where ID='$pid'");
unlink($imagepath);
...
<?php 
$pid=intval($_GET['id']);
$query=mysqli_query($con,"select tblrentalcloth.Image1,tblrentalcloth.Title from tblrentalcloth  where  tblrentalcloth.ID='$pid' ");
while($row=mysqli_fetch_array($query))
{
?>                                 
<form  method="post" enctype="multipart/form-data">                                

<input type="hidden" name="currentimage" value="<?php echo htmlentities($row['Image1']);?>">
...
<div class="col-4"><img src="productimages/<?php echo htmlentities($row['Image1']);?>" width="250"><br />
</div>
</div>

<div class="row" style="margin-top:1%;">
<div class="col-2"> New Featured Image</div>
<div class="col-4"><input type="file" name="image1" id="image1"  class="form-control" accept="image/*" title="Accept images only" required>
</div>
</div>
